...
attr_noise = [0] #np.linspace(0,1,5)

res = np.load('results/exp3_drf.npy') 
res_clf = np.load('results/exp3.npy')
print(res.shape) #y_flip, informative, reps, 49
print(res_clf.shape) #y_flip, informative, reps, 49

fig, ax = plt.subplots(6,5,figsize=(13,11), sharex=True, sharey=True)
drfs =  find_real_drift(150,2)
print(drfs)
